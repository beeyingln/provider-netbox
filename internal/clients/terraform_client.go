...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		if v, ok := clientCreds["allow_insecure_https"]; ok {
			ps.Configuration["allow_insecure_https"] = v
		}
		
		if v, ok := clientCreds["api_token"]; ok {
			ps.Configuration["api_token"] = v
		}
		
		if v, ok := clientCreds["headers"]; ok {
			ps.Configuration["headers"] = v
		}
		
		if v, ok := clientCreds["server_url"]; ok {
			ps.Configuration["server_url"] = v
		}
		
		if v, ok := clientCreds["skip_version_check"]; ok {
			ps.Configuration["skip_version_check"] = v
		}
